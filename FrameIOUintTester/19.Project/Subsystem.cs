...
namespace Project
{
    public partial class Subsystem
    {
        ...
        //通道声明
        public FioChannel Udp;
        
        //通道初始化
        public void InitialChannelUdp(ChannelOption ops)
        {
            if (ops == null) ops = new ChannelOption();
            if (!ops.Contains("localip")) ops.SetOption("localip", "127.0.0.1");
            if (!ops.Contains("localport")) ops.SetOption("localport", 10001);
            if (!ops.Contains("remoteip")) ops.SetOption("remoteip", "127.0.01");
            if (!ops.Contains("remoteport")) ops.SetOption("remoteport", 10002);
            ops.SetOption("$channeltype", 5);
            Udp = FioNetRunner.GetChannel(ops);
        }
        ...
        {
            var __v__ = FioNetRunner.RecvFrame("Subsystem_ReceiveFrame", Udp);
