...
namespace FrameIO.Run
{
    ...
    internal class SegmentCheckValidator : IValidate
    {
        ...
        public bool Valid(IFrameReadBuffer buff, SegRunNumber seg, JToken value)
        {
            var res = GetCheckResult(buff, value.Parent.Parent.Value<JObject>(), seg.Parent);
            var ret = (value.Value<ulong>() == res);
            if (!ret) ErrorInfo = "校验失败";
            return ret;
        }

        public ulong GetCheckResult(IFrameReadBuffer buff, JObject vParent, SegRunContainer segParent)
        {
            ...
            var i1 = buff.GetBytePos(first.Parent);
            var i2 = buff.GetBytePos(end.Parent) + (segParent[_end_seg].GetBitLen(vParent)/8);
